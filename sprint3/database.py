...
Basado en el diagrama de clases UML proporcionado.

Usa SQLAlchemy (ORM) con SQLite como motor de base de datos.
El archivo de la base de datos se crea automáticamente como 'reservas.db'
"""

from sqlalchemy import (
    create_engine, Column, Integer, String, Date, Time, ForeignKey, Table
)
...
# Motor de base de datos (archivo local SQLite)
engine = create_engine("sqlite:///reservas.db", echo=False)

# Fábrica de sesiones (con esto haces consultas y guardas datos)
...
class Espacio(Base):
    ...
    def consultar_disponibilidad(self, session, fecha: date, hora_inicio: time, hora_fin: time):
        """Retorna True si el espacio está libre en ese horario."""
        ocupadas = (
            session.query(Reserva)
            .filter(
                Reserva.espacios.contains(self),
                Reserva.fecha == fecha,
                Reserva.estado == "aprobada",
...
class Calificacion(Base):
    ...
    id         = Column(Integer, primary_key=True, autoincrement=True)
    puntuacion = Column(Integer, nullable=False)   # ej. 1 a 5
...
def crear_base_de_datos():
    """Crea todas las tablas en el archivo reservas.db"""
    Base.metadata.create_all(engine)
    print("✅ Base de datos creada exitosamente en 'reservas.db'")
